...
\begin{filecontents}[overwrite, nosearch, noheader]{randomnames-random-a.lua}
...
function Random.get_next_number(random, from, to)
  ...
  return from                      -- Navrať celé číslo
\end{filecontents}
\begin{filecontents}[overwrite, nosearch, noheader]{randomnames-random-b.lua}
       + math.floor( result        -- v zadaném rozsahu.\footnotemark
\end{filecontents}
\begin{filecontents}[overwrite, nosearch, noheader]{randomnames-random-c.lua}
                   * (to - from + 1))
end
\end{filecontents}

\begin{mintedblock}
\myinputminted{lua}{randomnames-random-a.lua}
\myinputminted[texcomments]{lua}{randomnames-random-b.lua}
\footnotetext{Jazyk Lua 5.3 používá 64bitová reálná čísla s 53bitovou mantisou~\cite[sekce~2.1]{ierusalimschy2020lua}. Díky tomu dostaneme při použití reálné aritmetiky vždy stejné výsledky nezávisle na platformě. Zvídavý čtenář se ale jistě zamyslí, jak bychom se tu mohli reálné aritmetice zcela vyhnout.}
\myinputminted{lua}{randomnames-random-c.lua}
\end{mintedblock}
...
\noindent
\begin{luacode*}
os.execute("cat randomnames-random-a.lua randomnames-random-b.lua randomnames-random-c.lua > randomnames-random.lua")
...
local function take_random_step(graph, context, random)
  if graph[context] == nil then  -- Pokud neexistuje vrchol pro
    return nil end               -- současný kontext, selži.
...
Při stahování příkladových databází používáme terminál \textsc{unix}ového systému s příkazovým procesorem \texttt{/bin/bash} a s nainstalovaným programem \texttt{xmllint} z knihovny libxml2. Příkladové dokumenty si můžeme uložit do textového souboru \texttt{priklad.tex} a přeložit příkazem \texttt{lualatex priklad.tex}. Pro úspěšný překlad stáhneme do pracovního adresáře také soubory \texttt{randomnames.lua}, \texttt{randomnames.tex} a \texttt{randomnames.sty}~\cite{novotny2023napadovnik}, které jsme vytvořili v sekcích~\ref{sec:implementace} a \ref{sec:rozhrani}.
