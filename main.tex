...
\bool_gset_false:N                    % ukládat jména, která
  \l_randomnames_save_bool            % využívá opakovaně.
...
\blfootnote{Znak vlnky (\texttt{\textasciitilde}) v jazyce expl3 vkládá mezeru, která nám v kódu v jazyce Lua odděluje klíčové slovo \texttt{local} od názvu proměnné \texttt{randomnames}. Pokud bychom vlnku neuvedli, definovali bychom místo lokální proměnné \texttt{randomnames} globální proměnnou \texttt{localrandomnames}.}
\myinputminted{tex}{randomnames-d.tex}
\end{mintedblock}

\begin{luacode*}
os.execute("cat randomnames-a.tex randomnames-b.tex randomnames-c.tex randomnames-d.tex > randomnames.tex")
\end{luacode*}
...
Při stahování příkladových databází používáme terminál \textsc{unix}ového systému s příkazovým procesorem \texttt{/bin/bash} a s nainstalovaným programem \texttt{xmllint} z knihovny libxml2. Příkladové dokumenty si můžete uložit do textového souboru \texttt{priklad.tex} a přeložit příkazem \texttt{lualatex priklad.tex}. Pro úspěšný překlad si stáhneme do pracovního adresáře také soubory \texttt{randomnames.lua}, \texttt{randomnames.tex} a \texttt{randomnames.sty}~\cite{novotny2023napadovnik}, které jsme vytvořili v sekcích~\ref{sec:implementace} a \ref{sec:rozhrani}.
...
\end{luacode*}
%
Stáhneme si proto z webu behindthename.com seznam \input|"wc -l < tolkien-jmena-trpasliku.txt" jmen trpaslíků, \input|"wc -l < tolkien-jmena-elfek.txt" jmen elfek a \input|"wc -l < tolkien-jmena-lidskych-muzu.txt" jmen lidských mužů.
...
parametr \verb|save|, kterým si jméno postavy trvale uložíme:
...
\input tolkien-jmena-example-02
\unskip\unskip\unskip\unskip\unskip\unskip
\exampleoutput{\input tolkien-jmena-example-06 \input tolkien-jmena-example-06}
...
\end{luacode*}
%
Stáhneme si proto z anglické wikipedie seznam \input|"wc -l < lovecraft-jmena-prastarych.txt" jmen prastarých:
...
\end{luacode*}
%
Stáhneme si proto z webu harrypotter.fandom.com seznam \input|"wc -l < harry-potter-kouzla.txt" kouzel:
...
\end{luacode*}
%
Stáhneme si proto z webu bulbapedia.bulbagarden.net seznam \input|"wc -l < jmena-pokemonu.txt" jmen pokémonů:
...
Pokud nemáme trénovací data v různých gramatických pádech a číslech, bude náš jazykový model generovat jména pouze v prvním pádě jednotného čísla. V analytickém jazyce, jako je angličtina, toto není problém. U flektivních jazyků, jako jsou čeština a slovenština, bychom ale chtěli, aby náš model uměl jména nejen generovat, ale také skloňovat.
...
Popsaný algoritmus funguje pouze pro česká jména a hledání jména s nejpodobnější koncovkou má lineární časovou složitost ve velikosti trénovací množiny. Budoucí práce by se měla zaměřit na rozšíření algoritmu o další flektivní jazyky jako slovenštiny a použít datovou strukturu jako trie pro efektivní hledání trénovacích jmen s nejdelší společnou koncovkou.
...
Popsané problémy souvisí se zapomnětlivostí našeho jazykového modelu a nemají přímočaré řešení, tj. bez úprav našeho modelu nelze permutermíny použít. Budoucí práce by se měla zaměřit na popis a implementaci těchto úprav.
...
\begin{equation}
P\big(S[i]\mid k\big) = \lambda\cdot P\big(S[i]\mid k; M_{\text{s}}\big) + (1-\lambda)\cdot \big(S[i]\mid k ; M_{\text{v}}\big),
\end{equation}
...
Při přípravě dokumentových šablon se nám může hodit generovat výplňový text, který do šablon nalijeme při přípravě ukázkových maket. Pokud budeme, stejně jako v předchozí sekci, trénovat naše modely místo jmen na větách, můžeme snadno generovat věrohodný výplňový text.
%
...
V tomto článku jsme si ukázali, jak můžeme pomocí jazykových modelů automaticky generovat jména postav při tvůrčím psaní v Lua\TeX u. Dále jsme vyvinuli balíček, který je přístupný laickým uživatelům \LaTeX u, a zveřejnili jsme ho online~\cite{novotny2023napadovnik}. Nakonec jsme si rozebrali možná vylepšení našeho modelu a další možné aplikace jazykových modelů v Lua\TeX u. Článek názorně ukazuje vývoj \LaTeX ového balíčku, který kombinuje kód v \TeX u s kódem v jazyce Lua, a demonstruje možnosti využití jazykových modelů v \TeX u.
